...
- (void)setImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder options:(SDWebImageOptions)options progress:(SDWebImageDownloaderProgressBlock)progressBlock completed:(SDWebImageCompletedBlock)completedBlock
{
    ...
    if (url)
    {
        float activity_width = 37;
        float activity_height = 37;
        
        __weak UIImageView *wself = self;
        
        UIActivityIndicatorView *activity = [[UIActivityIndicatorView alloc] initWithFrame:CGRectMake(self.frame.size.width / 2 - activity_width / 2, self.frame.size.height / 2 - activity_height / 2, activity_width, activity_height)];
...
- (void)cancelCurrentImageLoad
{
    ...
    if (operation)
    {
        ...
        for (UIActivityIndicatorView *activity in [self subviews]) {
            [activity stopAnimating];
            [activity removeFromSuperview];
        }
